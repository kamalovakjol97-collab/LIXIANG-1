...
const Services = () => {
  ...
  const handleKeyDown = useCallback((e, index) => {
    if (e.key === 'ArrowDown' || e.key === 'ArrowRight') {
      e.preventDefault()
      goTo(Math.min(index + 1, currentServices.length - 1))
      ...
      e.preventDefault()
      goTo(currentServices.length - 1)
    }
  }, [currentServices.length, goTo])

  return (
    <section className="services-section" aria-label={t('services')}>
      <div className="container">
        <h2 className="services-section-title">{t('services')}</h2>

        <div className="services-two-column">
          <nav
            className="services-nav"
            role="tablist"
            aria-label={language === 'ru' ? 'Список услуг' : '服务列表'}
          >
            {currentServices.map((service, index) => (
              <button
                ...
                className={`service-nav-item ${activeIndex === index ? 'service-nav-item--active' : ''}`}
                onClick={() => goTo(index)}
                onKeyDown={(e) => handleKeyDown(e, index)}
              >
                <span className="service-nav-number" aria-hidden="true">
                  {String(index + 1).padStart(2, '0')}
                </span>
                <span className="service-nav-content">
                  <span className="service-nav-title">{service.title}</span>
                  <span className="service-nav-teaser">{service.description}</span>
                </span>
              </button>
            ))}
          </nav>
          ...
          >
            <div
              className={`services-detail-inner ${isTransitioning ? 'services-detail-inner--fade' : ''}`}
              key={`${activeService.id}-${language}`}
            >
              <h3 className="services-detail-title">{activeService.title}</h3>
              <p className="services-detail-description">{activeService.description}</p>
              <ul className="services-detail-subs">
                {activeService.subs.map((sub, i) => (
                  <li key={i}>{sub}</li>
                ))}
              </ul>
              <button
                className="service-btn services-detail-cta"
